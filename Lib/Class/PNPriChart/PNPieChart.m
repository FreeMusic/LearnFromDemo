...
//    _outerCircleRadius  = CGRectGetWidth(self.bounds) / 3;
//    _innerCircleRadius  = CGRectGetWidth(self.bounds) / 6;
//    _descriptionTextColor = [UIColor redColor];
...
//        titleValue = [NSString stringWithFormat:@"%.0f",currentDataItem.value];//圈上的文字
    }else{
...
- (void)maskChart{
    CGFloat radius = _innerCircleRadius + (_outerCircleRadius - _innerCircleRadius);
    CGFloat borderWidth = _outerCircleRadius - _innerCircleRadius;
...
//    }
    ...
    if (self.shouldHighlightSectorOnTouch)
    {
        ...
        //点击后的模块形状
        self.sectorHighlight = [self newCircleLayerWithRadius:_outerCircleRadius + 10
                                                  borderWidth:20
                                                    fillColor:[UIColor clearColor]
