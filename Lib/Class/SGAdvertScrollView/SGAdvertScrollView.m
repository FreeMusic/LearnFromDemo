...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.titlColor != nil) {
        cell.tipsLabel.textColor = self.titlColor;
    }
    return cell;
}
...
- (void)setImage:(UIImage *)image {
    _image = image;
    _imageView.image = [UIImage imageNamed:@"tongzhi"];
}

- (void)setTitleArray:(NSArray *)titleArray {
    NSMutableArray *array = [[NSMutableArray alloc] initWithArray:titleArray];
    if (titleArray.count < 3) {
        if (titleArray.count) {
//            [array addObject:titleArray[0]];
        }
    }
    _titleArray = array;
    self.tempArr = [NSArray arrayWithArray:array];
}
