...
+ (instancetype)dialogWithTitle:(NSString *)title messages:(NSString *)string attrs:(NSDictionary *)attrs commitCallBack:(commitClickBlock)commitCallBack;
{
    NSUserDefaults *user = [NSUserDefaults standardUserDefaults];
    [user setObject:string forKey:@"message"];
    [user synchronize];//把中间信息存入本地
    ...
    dialogFrameView.backgroundColor = XFDialogRealValueForRef(attrs, XFDialogBackground, [UIColor whiteColor]);//弹出框的背景颜色
    ...
    titleLabel.textColor = [UIColor blackColor];//头部字体的颜色
    titleLabel.font = [UIFont systemFontOfSize:45*m6Scale];
    titleLabel.textAlignment = XFDialogRealValueWithTypeForRef(attrs,XFDialogTitleAlignment,intValue,NSTextAlignmentCenter);//居中
    if (attrs[XFDialogTitleIsMultiLine]) {//换行
