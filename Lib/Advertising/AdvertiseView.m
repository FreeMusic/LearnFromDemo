...
@property (nonatomic, strong) FLAnimatedImageView *adView;
...
- (instancetype)initWithFrame:(CGRect)frame{
    if (self = [super initWithFrame:frame]) {
        ...
        // 1.广告图片
        _adView = [[FLAnimatedImageView alloc]initWithFrame:frame];
...
- (void)setFilePath:(NSString *)filePath
{
    ...
    [_adView sd_setImageWithURL:url];
}
- (void)pushToAd{
...
- (void)dismiss
{
    //当广告页消失的时候，发送通知 去判断用户是否开启了指纹解锁  若开启则会加载指纹解锁界面
    NSNotification *notification = [[NSNotification alloc] initWithName:@"Finger" object:nil userInfo:nil];
...
- (void)dealloc{
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"Finger" object:nil];
}
