...
@interface AwardDetailsVC ()<UITableViewDelegate, UITableViewDataSource, FactoryDelegate>
...
- (void)viewDidLoad {
    ...
    for (int i = 0; i < array.count; i++) {
        ...
        [label mas_makeConstraints:^(MASConstraintMaker *make) {
            if (i) {
                if (i == array.count-1) {
                    make.width.mas_equalTo(kScreenWidth/array.count-33*m6Scale);
                    make.left.mas_equalTo((i-1)*(kScreenWidth/array.count-33*m6Scale)+kScreenWidth/array.count+100*m6Scale);
                }else{
                    make.width.mas_equalTo(kScreenWidth/array.count-33*m6Scale);
                    make.left.mas_equalTo((i-1)*(kScreenWidth/array.count-33*m6Scale)+kScreenWidth/array.count+80*m6Scale);
                }
            }else{
                make.width.mas_equalTo(kScreenWidth/array.count+80*m6Scale);
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    AwardDetailsCell *cell = [tableView dequeueReusableCellWithIdentifier:reuse];
    if (!cell) {
        cell = [[AwardDetailsCell alloc] initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:reuse];
    }
...
- (void)serviceData{
    ...
    if (state == 0) {
        ...
        [DownLoadData postGetNewRewardLists:^(id obj, NSError *error) {
            //数据请求失败
