...
@interface BuyOrderVC ()<UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate>
...
- (void)GetData{
    ...
    [DownLoadData postGetOrderDetails:^(id obj, NSError *error) {
        ...
        //总计
        [self.dataArr addObject:[NSString  stringWithFormat:@"总计：￥%.1f", _model.orderAmount.floatValue*(_model.goodsNum.floatValue)]];
        //联系人地址
        ...
        //应付金额
        _accountLabel.text = [NSString stringWithFormat:@"应付金额：￥%.2f", _model.orderAmount.floatValue];
        NSString *address = [NSString stringWithFormat:@"%@", _model.address];
        NSString *name = [NSString stringWithFormat:@"%@", _model.receiveName];
        NSString *mobile = [NSString stringWithFormat:@"%@", _model.receiveMobile];
        
        if (![address isEqualToString:@"(null)"] && ![mobile isEqualToString:@"(null)"] && ![name isEqualToString:@"(null)"]) {
            [self.addressArr addObject:[NSString stringWithFormat:@"%@", _model.address]];
            [self.addressArr addObject:[NSString stringWithFormat:@"%@", _model.receiveName]];
            [self.addressArr addObject:[NSString stringWithFormat:@"%@", _model.receiveMobile]];
        }
...
- (void)cancleButtonClick{
    ...
    [alert addAction:[UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        } orderNo:[NSString stringWithFormat:@"%@", _model.orderNo]];
    }]];
...
- (void)PayButtonClick{
    if (self.addressArr.count) {
        ...
        self.rechargeView.amountLabel.text = [NSString stringWithFormat:@"￥%.2f", _model.orderAmount.floatValue];
        ...
        if ([obj[@"result"] isEqualToString:@"fail"]) {
            ...
            dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    tempVC.type = 3;//订单确认页面的数据获取方式是顺值(从我的订单  未完成订单跳转)
                    tempVC.orderNo = [NSString stringWithFormat:@"%@", _model.orderNo];//商品定单编号
                    [self.navigationController pushViewController:tempVC animated:YES];
                });
            });
        }
    } orderNo:[NSString stringWithFormat:@"%@", _model.orderNo] address:[NSString stringWithFormat:@"%@", self.addressArr[0]] receiveName:[NSString stringWithFormat:@"%@", self.addressArr[1]] receiveMobile:[NSString stringWithFormat:@"%@", self.addressArr[2]] password:passWord salt:mcryptKey];
}
/**
 *提示用户交易密码输入错误
 */
...
- (void)inquiryUserAddress{
    ...
    [DownLoadData postQueryAddressByUserId:^(id obj, NSError *error) {
        ...
        if (![address isEqualToString:@"(null)"] && ![mobile isEqualToString:@"(null)"] && ![name isEqualToString:@"(null)"]) {
            [self.addressArr addObject:[NSString stringWithFormat:@"%@", model.address]];
            [self.addressArr addObject:[NSString stringWithFormat:@"%@", model.userName]];
            [self.addressArr addObject:[NSString stringWithFormat:@"%@", model.mobile]];
        }
