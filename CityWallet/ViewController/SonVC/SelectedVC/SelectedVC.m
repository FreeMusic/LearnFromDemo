...
@interface SelectedVC ()<UITableViewDelegate, UITableViewDataSource, FactoryDelegate>
...
- (void)serviceData{
    ...
    if (state == 0) {
        ...
        } pageNum:@"1" pageSize:@"10" itemStatus:self.itemStatus itemType:@"-1"];
    }
}
#pragma mark - 上拉加载数据
- (void)pullup{
...
- (void)loadData{
    ...
    if (state == 0) {
        ...
        } pageNum:[NSString stringWithFormat:@"%ld", (long)_page] pageSize:@"10" itemStatus:self.itemStatus itemType:@"-1"];
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    CityWalletCell *cell = [tableView dequeueReusableCellWithIdentifier:SelectedVC];
    if (!cell) {
        cell = [[CityWalletCell alloc] initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:SelectedVC];
    }
            ...
            [alert addAction:[UIAlertAction actionWithTitle:@"开启" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                ...
                [DownLoadData postAppointBid:^(id obj, NSError *error) {
                    if ([obj[@"result"] isEqualToString:@"success"]) {
                        ...
                        NSLog(@"推送时间==%f",[self.appointTime doubleValue]);
...
- (void)LocalNotice :(NSString *)itemId{
    if ([[UIDevice currentDevice].systemVersion floatValue] >= 10.0) {
        ...
        [center addNotificationRequest:request withCompletionHandler:^(NSError * _Nullable error) {
            NSLog(@"添加了10推送啊==%f",[self.appointTime doubleValue]);
        }];
 ...
 *开抢时间
 */
- (NSString *)startTimeBydata:(NSNumber *)data{
    //标几点开抢
    NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
    [formatter setDateStyle:NSDateFormatterMediumStyle];
    [formatter setTimeStyle:NSDateFormatterShortStyle];
    [formatter setDateFormat:@"HH:mm"]; // ----------设置你想要的格式,hh与HH的区别:分别表示12小时制,24小时制
    NSDate *confromTimesp = [NSDate dateWithTimeIntervalSince1970:data.integerValue/1000];
    NSString *dataStr = [NSString stringWithFormat:@"%@", confromTimesp];
