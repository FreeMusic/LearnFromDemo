...
@interface CarCreditVC ()<UITableViewDelegate, UITableViewDataSource, FactoryDelegate>
...
- (void)serviceData{
    ...
    if (state == 0) {
        ...
        } pageNum:@"1" pageSize:@"10" itemStatus:self.itemStatus itemType:@"3"];
    }
}
#pragma mark - 上拉加载数据
- (void)pullup{
...
- (void)loadData{
    ...
    if (state == 0) {
        ...
        } pageNum:[NSString stringWithFormat:@"%ld", (long)_page] pageSize:@"10" itemStatus:self.itemStatus itemType:@"3"];
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    static NSString *CarCreditVC = @"CarCreditVC";
    CityWalletCell *cell = [tableView dequeueReusableCellWithIdentifier:CarCreditVC];
    if (!cell) {
        cell = [[CityWalletCell alloc] initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:CarCreditVC];
    }
...
- (void)LocalNotice :(NSString *)itemId{
    NSLog(@"版本==%f",[[UIDevice currentDevice].systemVersion floatValue]);
    if ([[UIDevice currentDevice].systemVersion floatValue] >= 10.0) {
        ...
        [center addNotificationRequest:request withCompletionHandler:^(NSError * _Nullable error) {
            NSLog(@"添加了10推送==%f",[self.appointTime doubleValue]);
